...
function Stock() {
  ...
  const handleStatusChange = (id, status, type, quantity) => {
    ...
    axios.put(`https://backend-iuq5.vercel.app/updateStatus/${id}`, { status: newStatus, type, quantity })
      .then(res => {
  ...
  const handleTypeChange = async (id) => {
    try {
      await axios.put(`https://backend-iuq5.vercel.app/updateType/${id}`, { type: 'Returned' });
      fetchStocks(department, selectedLocation, location.state?.statusFilter); // Refresh the stock list
    } catch (err) {
