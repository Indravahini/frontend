...
const RequestPopup = ({ department }) => {
    ...
    useEffect(() => {
        axios.get('https://backend-iuq5.vercel.app/api/locations')
    ...
    useEffect(() => {
        if (isPopupOpen) {
            axios.get('https://backend-iuq5.vercel.app/api/requests', {
    ...
    const handleSubmit = async () => {
        try {
            await axios.post('https://backend-iuq5.vercel.app/api/approve-requests', { approvals: selectedRequests });
            handleClose(); 
        } catch (err) {
